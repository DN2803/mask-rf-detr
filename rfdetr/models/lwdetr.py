...
from rfdetr.util.misc import (NestedTensor, nested_tensor_from_tensor_list,
                       ...
                       is_dist_avail_and_initialized, interpolate)
...
class SetCriterion(nn.Module):
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        src_masks = src_masks[src_idx]  # [num_queries_matched, H_pred, W_pred]

        # Lấy kích thước của mask dự đoán
        pred_h, pred_w = src_masks.shape[-2:]

        # Lấy mask ground truth
        masks = [t["masks"] for t in targets]  # List of [num_gt_boxes, H_gt, W_gt]
        target_masks, valid = nested_tensor_from_tensor_list(masks).decompose()
        target_masks = target_masks.to(src_masks.device)
        target_masks = target_masks[tgt_idx]

        # Resize GT mask về đúng kích thước của prediction mask
        target_masks = interpolate(target_masks[:, None], size=(pred_h, pred_w),
                                mode="bilinear", align_corners=False)[:, 0]

        # Làm phẳng để tính loss
        src_masks = src_masks.flatten(1)
        target_masks = target_masks.flatten(1)
        target_masks = target_masks.view_as(src_masks)
